...
if (isset($_POST['add_expense']) && $active_trip_id) {
    ...
    if ($db->openConnection()) {
        ...
        if (!$validCat) {
            ...
            if ($db->insert($query)) {
                ...
                $error = "DB insert failed: " . $db->connection->error;
            }
        }
        $db->closeConnection();
    } else {
    ...
    $redirect = "expenses.php?trip_id=$active_trip_id";
    if (isset($success)) $redirect .= "&saved=expense";
    if (isset($error)) $redirect .= "&error=" . urlencode($error);
    header("Location: $redirect");
    exit;
}

// ── Load categories for dropdown ─────────────────────────────────────
$dbCat = new DBController();
$categories = [];
if ($dbCat->openConnection()) {
    $catResult = $dbCat->select("SELECT category_id, name FROM category ORDER BY name");
    $categories = $catResult ? $catResult : [];
    $dbCat->closeConnection();
}
...
if ($expenseModel) {
    ...
    // If the model doesn't join category, manually add names
    $dbName = new DBController();
    if ($dbName->openConnection()) {
        foreach ($expenses as &$exp) {
            $res = $dbName->select("SELECT name FROM category WHERE category_id = " . (int)$exp['category_id']);
            $exp['category_name'] = $res ? $res[0]['name'] : '?';
        }
        $dbName->closeConnection();
    }
}

// Handle Non-Cash Submission
$nonCashMessage = "";
$nonCashStatus = ""; 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_non_cash'])) {
    ...
    if ($conn) {
        // Turn autocommit OFF for safety (if not already)
        ...
        $sql = "INSERT INTO non_cash_contribution (trip_id, contributor_id, estimatedValue, description, proof_file, status) 
                VALUES (?, ?, ?, ?, ?, 'pending')";
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                $conn->commit();   // ✅ THIS MAKES IT PERSIST (same as Alert pattern)
            ...
            $nonCashMessage = "Prepare failed: " . $conn->error;
            $nonCashStatus = "error";
        }
        
        $db->closeConnection();
    } else {
        $nonCashMessage = "Could not connect to database.";
        $nonCashStatus = "error";
    }
}
    ...
    <select name="category_id" class="input" required>
      ...
      <?php if ($nonCashMessage): ?>
         <div class="card" style="margin-bottom:1rem; padding:0.75rem; 
               background: <?php echo $nonCashStatus === 'success' ? 'rgba(34,197,94,0.12)' : 'rgba(239,68,68,0.12)'; ?>; 
      ...
      <div class="card" style="margin-bottom:1rem;">
         <h3 class="card__title">📦 Non-cash contributions</h3>
            <form method="POST" enctype="multipart/form-data" class="form-grid" style="margin-top:0.75rem;">
